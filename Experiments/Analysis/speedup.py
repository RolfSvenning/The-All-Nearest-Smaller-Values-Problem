...
def plotSpeedup(E, title, logScale, showSpeedup=False):
    _, ax = plt.subplots()
    ...
    if showSpeedup: Tnorm = np.array([T1 / e.averageTime for e in E])
    ...
    for algorithmType, color in AtoC.items():
        ...
        if showSpeedup: Pall, Tall = zip(*[(e.numberOfCores, T1 / t) for e in np.array(E)[ix] for t in e.times])
        ...
        ax.plot(P2, Tnorm2, c=color, label=algorithmType + " " + algorithmType)
...
plotSpeedup(Es, "Number of Processors vs. T1 / Average Running Time (blocksize = 256 * log2(n))", logScale=False, showSpeedup=True)
